...
\begin{itemize}
    \item \textbf{Data Enrichment:} Getting more samples for the SNII class to fix the imbalance.
    \item \textbf{Dynamic Windows:} We will explore different time windows. Instead of a fixed 100-day window, using "dynamic windows" could help us identify Supernovae faster and not confuse them with long-term objects like QSOs.
    \item \textbf{New Architectures:} Testing if adding "Attention" mechanisms helps the TCN focus on the most important parts of the light curve.
\end{itemize}
